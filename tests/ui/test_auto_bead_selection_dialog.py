...
def _build_test_image() -> tuple[np.ndarray, tuple[int, int, int, int]]:
    ...
    image[20:28, 6:14] = template
    image[28:36, 28:36] = template
    return image, seed_roi


def test_auto_bead_selection_dialog_updates_preview_with_percentile(qtbot):
    ...
    dialog.threshold_slider.setValue(100)

    assert len(dialog.visible_candidates) >= 1
    assert dialog.accept_button.isEnabled()
    assert 'score >=' in dialog.status_label.text()
